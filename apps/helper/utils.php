...
class utils
{
    ...
    {
        $this->DB->query("SELECT sum(simpanan_sukarela.jumlah) as saldo from simpanan_sukarela WHERE simpanan_sukarela.anggota=:nik GROUP BY simpanan_sukarela.anggota");
        $this->DB->bind('nik', $nik);
        return $this->DB->single();
    }
    function getSaldoTerbesar($nik)
    {
        $this->DB->query("SELECT saldo from simpanan_sukarela WHERE simpanan_sukarela.anggota=:nik ORDER BY saldo DESC LIMIT 1");
        $this->DB->bind('nik', $nik);
        return $this->DB->single();
    }
